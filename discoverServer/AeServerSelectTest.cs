    ...
    public static class AeServerSelectTest
    {
        ...
        public static void FindNotifiersRecursive(OpcNodeInfo node, int depth, HashSet<string> visited)
        {
            ...
            try
            {
                ...
                {
                    Console.WriteLine($"  [HasNotifier] {notifier.DisplayName} = {notifier.NodeId} (父: {node.DisplayName})");
                }

                // 继续递归所有子节点
                foreach (var child in node.Children())
                {
                    FindNotifiersRecursive(child, depth + 1, visited);
                }
            }
            catch { }
        }
        ...
        public static void FindEventSourcesRecursive(OpcNodeInfo node, int depth, HashSet<string> visited)
        {
            ...
            try
            {
                ...
                {
                    Console.WriteLine($"  [HasEventSource] {source.DisplayName} = {source.NodeId} (父: {node.DisplayName})");
                }

                // 继续递归所有子节点
                foreach (var child in node.Children())
                {
                    FindEventSourcesRecursive(child, depth + 1, visited);
                }
            }
            catch { }
        }
        ...
        public static void FindConditionsRecursive(OpcNodeInfo node, int depth, HashSet<string> visited)
        {
            ...
            try
            {
                ...
                {
                    Console.WriteLine($"  [HasCondition] {condition.DisplayName} = {condition.NodeId} (父: {node.DisplayName})");
                }

                // 继续递归所有子节点
                foreach (var child in node.Children())
                {
                    FindConditionsRecursive(child, depth + 1, visited);
                }
            }
            catch { }
        }
